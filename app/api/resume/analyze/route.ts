		...
		if (file.type === 'application/pdf') {
			try {
				...
				if (!fileContent || fileContent.trim().length === 0) {
					return NextResponse.json(
						{ error: 'Could not extract text from PDF. Please ensure the PDF contains readable text and try again.' },
						{ status: 400 }
					);
				}
			} catch (pdfError) {
				...
				return NextResponse.json(
					{ error: 'Failed to parse PDF file. Please try uploading a different format or ensure the PDF is not corrupted.' },
					{ status: 400 }
				);
			}
		} else {
...
Provide specific, actionable feedback that will help improve the resume's effectiveness.`,
		...
		try {
			...
			try {
				...
				const strengthsMatch = text.match(/strengths?[:\s]*(.*?)(?=improvements?|suggestions?|summary|$)/is);
				const improvementsMatch = text.match(/improvements?[:\s]*(.*?)(?=strengths?|suggestions?|summary|$)/is);
				const suggestionsMatch = text.match(/suggestions?[:\s]*(.*?)(?=strengths?|improvements?|summary|$)/is);
				...
				analysis = {
					...
					suggestions: suggestionsMatch ? suggestionsMatch[1].split(/[,\n]/).map(s => s.trim()).filter(s => s) : [
